...
Updated with proper Finnhub integration
"""
...
try:
    ...
    print("⚠️  Finnhub library not installed. Run: pip install finnhub-python")
...
class NewsAnalyzer:
    ...
    def __init__(self, alpha_vantage_key: Optional[str] = None, finnhub_key: Optional[str] = None):
        ...
        if FINNHUB_AVAILABLE and self.finnhub_key:
            ...
            print("⚠️  Finnhub library not available - install with: pip install finnhub-python")
        elif not self.finnhub_key:
            print("⚠️  Finnhub API key not provided")
    ...
    def _get_finnhub_news(self, symbol: str, days_back: int) -> List[Dict]:
        """Fetch news from Finnhub API using proper client"""
        news_data = []
        ...
        try:
            ...
            # Use Finnhub client to get company news
            news_response = self.finnhub_client.company_news(
                symbol, 
            ...
            if news_response:
                ...
                print(f"✅ Fetched {len(news_data)} articles from Finnhub for {symbol}")
